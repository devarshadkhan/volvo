...
const Tickets = () => {
  ...
  useEffect(() => {
    if (values.ticketNo || values.searchValue || values.teamId || values.searchStatus || values.startDate || values.endDate) {
  ...
  }, [pageNumber,addTicket.success,ticketStatus.success,updateUser.success,answerTicketAPi.success]);
  ...
  const {
    ...
    onSubmit: (data) => {
      ...
      if (!data.ticketNo && !data.searchValue && !data.teamId && !data.searchStatus && !data.startDate &&  !data.endDate ) {
              ...
              <div className="row">
                ...
                {isPermitted(permitted?.isSearch) && (
                  <div class="form-group ">
                    <input type="text" class="input-control" name="ticketNo" placeholder="Ticket Number"
                      onChange={handleChange} value={values.ticketNo} />
                ...
                {isPermitted(permitted?.isSearch) && (
                  <div class="form-group ">
                    <input type="text" class="input-control" name="searchValue" placeholder="Question"
                     onChange={handleChange} value={values.searchValue} />
                ...
                {isPermitted(permitted?.isSearch) && ( 
                  <div class="form-group ">
                    <select class="input-control" onChange={handleChange} name="teamId" value={values.teamId}> {" "}
                      <option value="" selected>Select Team</option>
                      {Array.isArray(allTeams.teams) && allTeams.teams.length > 0 ? (
                ...
                {isPermitted(permitted?.isSearch) && ( 
                  <div class="form-group ">
                    <select class="input-control" onChange={handleChange} name="searchStatus" value={values.searchStatus}> {" "}
                ...
                {isPermitted(permitted?.isSearch) && ( 
                  <div class="ticketDate">
                    <input   className="input-control" type="date" onChange={handleChange} name="startDate" id="startDate" value={values.startDate} />
                    <span>To</span>
                    <input   className="input-control" type="date" onChange={handleChange} name="endDate" id="endDate" value={values.endDate} />
                  </div>
                 )}     
                </div>
              </div>
                  ...
                  <div className="row">
                    ...
                    {isPermitted(permitted?.isCsv) && (
                      <button
                        ...
                        type="button"
                      >
                        <img src="/icons-images/download-img.png" alt="icon" />
                      </button>
                    )}
                  ...
                  ticketList?.dataItems?.map((user, index) => {
                    ...
                    return (
                      <tr key={index}>
                        ...
                        <td data-label="Ticket No.">{user.ticketNo ? user.ticketNo.replace(/#/g, '') : "NA"}</td>
                        <td data-label="Question" style={{ maxWidth: "200px",overflow: "hidden",textOverflow: "ellipsis"}}>{user.taskQuestion || "NA"}</td>
                        <td data-label="Team Leader">{user?.teamName || "NA"}</td>
                        <td data-label="Manager">{user.managerName || "NA"}</td>
                        <td data-label="Team ">{user.tlName || "NA"}</td>
                        <td data-label="Agent">{user?.agentName || "NA"}</td>
                        <td data-label="Start Date ">{user.createdAt || "NA"}</td>
                        {isPermitted(permitted?.isStatus) && (<td data-label="Status" className={`tdGape 
                          ${user.status == "0" ? "status-open" : user.status === "1" ? "status-close" : "status-pending"}`}>
                        ...
                        {(isPermitted(permitted?.isUpdate) ||
                          ...
                          <td data-label="Action">
                            {isPermitted(permitted?.isUpdate) && user.status != '2' &&(
                              <button className=" btn-small greenbg" type="button"
                                onClick={() => {
                                  setAnswerTicket(true);
                                  dispatch(getTicketByIdAction(user.ticketNo));
                                }}>
                                <img src={`${process.env.PUBLIC_URL}/icons-images/question-icon.png`} alt="icon" />
                              </button>
                            )}
                            {isPermitted(permitted?.isUpdate) && user.status != '2' && (
                              <button className=" btn-small bluebg" type="button"
                                onClick={() => {
                                  setUpdateTicket(true);
                                  dispatch(getTicketByIdAction(user.ticketNo));
                                }}>
                                <img src={`${process.env.PUBLIC_URL}/icons-images/ticket-icon.png`} alt="icon"/>
                              </button>
                            )}
                            {isPermitted(permitted?.isRead) && (
                              <button className=" btn-small yellowbg"
                                ...
                                <img src={`${process.env.PUBLIC_URL}/icons-images/view-icon.png`} alt="icon" className="eye-icon"/>
                              </button>
                            )}
                          </td>
                        )}
